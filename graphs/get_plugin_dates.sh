...
for aa in $(find . -name plugin.sh | grep -v cookie | grep -v 'deb' | sed -n 's/^\([^ ]*\)\/devstack\/plugin\.sh/\1/p'); do
...
for aa in $(find . -name upgrade.sh | grep -v cookie | grep -v 'deb' | sed -n 's/^\([^ ]*\)\/devstack\/upgrade\/upgrade\.sh/\1/p'); do
